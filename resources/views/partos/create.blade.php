<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('partos.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-lg border border-gray-200 shadow-sm transition-colors duration-150">
                Volver a la Lista
            </a>
        </div>
    </x-slot>
                ...
                <form action="{{ route('partos.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha_parto" id="fecha_parto" value="{{ old('fecha_parto', date('Y-m-d')) }}" required class="w-full text-sm rounded-lg border-gray-300 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                            @error('fecha_parto')
                                <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="peso_camada" id="peso_camada" step="0.01" min="0" value="{{ old('peso_camada') }}" placeholder="Opcional (ej. 15.5)" class="w-full text-sm rounded-lg border-gray-300 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                            @error('peso_camada')
                                <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="bg-gray-50/50 p-4 border border-gray-200 rounded-xl">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="lechones_vivos" id="lechones_vivos" min="0" value="{{ old('lechones_vivos', 0) }}" required class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <input type="number" name="lechones_muertos" id="lechones_muertos" min="0" value="{{ old('lechones_muertos', 0) }}" required class="w-full text-sm rounded-lg border-gray-300 focus:border-rose-500 focus:ring focus:ring-rose-200 focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <input type="number" name="lechones_momificados" id="lechones_momificados" min="0" value="{{ old('lechones_momificados', 0) }}" required class="w-full text-sm rounded-lg border-gray-300 focus:border-amber-500 focus:ring focus:ring-amber-200 focus:ring-opacity-50">
                    ...
                    <div>
                        ...
                        <textarea name="observaciones" id="observaciones" rows="4" placeholder="Registrar detalles adicionales (complicaciones del parto, asistencia veterinaria, condiciones de la jaula de maternidad, etc.)..." class="w-full text-sm rounded-lg border-gray-300 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50">{{ old('observaciones') }}</textarea>
                        @error('observaciones')
                            <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
