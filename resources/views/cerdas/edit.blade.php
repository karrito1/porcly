<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('cerdas.show', $cerda->id) }}" class="text-gray-500 hover:text-gray-900">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <h2 class="font-bold text-xl text-gray-800 leading-tight">
                {{ __('Editar Cerda — ' . $cerda->codigo) }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                    <h3 class="text-sm font-bold text-gray-800 uppercase tracking-wider">Modificar Datos de la Cerda</h3>
                    <p class="text-xs text-gray-500 mt-1">Realiza los cambios necesarios y presiona guardar.</p>
                </div>

                <form action="{{ route('cerdas.update', $cerda->id) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="estado" id="estado" class="w-full text-sm rounded-lg border-gray-250 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50 @error('estado') border-rose-300 focus:border-rose-500 focus:ring-rose-200 @enderror" required>
                                ...
                                <option value="en_celo" {{ old('estado', $cerda->estado) === 'en_celo' ? 'selected' : '' }}>En Celo</option>
                                <option value="descarte" {{ old('estado', $cerda->estado) === 'descarte' ? 'selected' : '' }}>Descarte (Inactiva)</option>
                            </select>
                            @error('estado')
                                <p class="text-rose-600 text-xs mt-1.5 font-medium">{{ $message }}</p>
                            @enderror
