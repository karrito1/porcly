<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('inseminaciones.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-500 hover:bg-brand-650 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors duration-200" style="background-color: #f4b08a;">
            ...
            <div class="bg-white border border-gray-200/80 rounded-2xl shadow-sm p-6 mb-6">
                <form action="{{ route('inseminaciones.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="buscar" id="buscar" value="{{ request('buscar') }}" placeholder="Código de la cerda (ej. C-001)..." class="w-full text-sm rounded-lg border-gray-250 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="resultado" id="resultado" class="w-full text-sm rounded-lg border-gray-250 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->anyFilled(['buscar', 'resultado']))
                            <a href="{{ route('inseminaciones.index') }}" class="w-full inline-flex justify-center items-center px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-750 text-xs font-bold rounded-lg border border-gray-200 shadow-sm transition-colors duration-150">
                                Limpiar
                            </a>
                        @endif
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($inseminaciones as $inseminacion)
                                <tr class="hover:bg-gray-50/40 transition-colors" x-data="{ showConfirmModal: false }">
                                    ...
                                    <td class="py-4 px-6 text-sm text-gray-750">
                                    ...
                                    <td class="py-4 px-6 text-right text-sm">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('cerdas.show', $inseminacion->cerda_id) }}" class="inline-flex items-center px-3 py-1 bg-white hover:bg-gray-50 text-gray-750 text-xs font-semibold border border-gray-200 rounded-lg transition-colors shadow-2xs">
                                                Ver Ficha Cerda
                                            </a>
                                            @if(is_null($inseminacion->exitosa))
                                                <button @click="showConfirmModal = true" class="inline-flex items-center px-3 py-1 bg-brand-100 hover:bg-brand-200 text-brand-700 text-xs font-semibold border border-brand-250 rounded-lg transition-colors shadow-2xs" style="background-color: #fee2e2; border-color: #fca5a5; color: #b91c1c;">
                                                    Confirmar Gestación
                                                </button>
                                            @endif
                                        </div>
                                                                ...
                                                                <div class="mt-4">
                                                                    <form action="{{ route('inseminaciones.confirm', $inseminacion->id) }}" method="POST" class="space-y-4 text-left">
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <textarea name="notas" id="notas" rows="3" placeholder="Detalles sobre el diagnóstico (ej. Ecografía realizada, celo recurrente...)" class="w-full text-sm rounded-lg border-gray-250 focus:border-brand-500 focus:ring focus:ring-brand-200 focus:ring-opacity-50"></textarea>
                                                                        </div>
                                                                        <div class="flex justify-end gap-3 pt-3 border-t border-gray-100">
                                                                            ...
                                                                            <button type="submit" class="px-4 py-2 text-xs font-bold text-white rounded-lg hover:bg-brand-650 transition-colors" style="background-color: #f4b08a;">
                                                                                Guardar Diagnóstico
                                                                            </button>
                                                                        </div>
                                                                    </form>
